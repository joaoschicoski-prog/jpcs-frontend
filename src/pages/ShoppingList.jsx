...
export default function ShoppingList({ setPage }) {
  ...
  const totalChecked = checked + manualChecked;
  const totalItems = list.length + manualItems.length;
  ...
  const isEmpty = list.length === 0 && manualItems.length === 0;
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <div style={{ background: "var(--green-600)", padding: "20px 16px 24px" }}>
        ...
        {totalItems > 0 && <p style={{ fontSize: 13, color: "rgba(255,255,255,0.75)" }}>{totalChecked} de {totalItems} item{totalItems !== 1 ? "s" : ""} marcado{totalChecked !== 1 ? "s" : ""}</p>}
      </div>
      ...
      <div style={{ padding: "12px 16px" }}>
        {isEmpty ? (
          <div style={{ textAlign: "center", padding: "48px 0", color: "var(--gray-500)" }}>
            ...
            <p style={{ fontSize: 13 }}>Adicione produtos ao navegar pelas ofertas ou escreva itens manualmente abaixo.</p>
          ...
          <>
            ...
            {list.length > 0 && (
              <>
                ...
                {/* HEADER PRODUTOS EM OFERTA */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <div style={{ width: 10, height: 10, borderRadius: "var(--radius-full)", background: "var(--green-500)", flexShrink: 0 }} />
                  <p style={{ fontSize: 12, fontWeight: 700, color: "var(--green-700)", margin: 0 }}>Produtos em oferta ({list.length})</p>
                </div>
            ...
            {/* SEÇÃO OUTROS ITENS */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, marginTop: list.length === 0 ? 0 : 8 }}>
              ...
              {manualItems.map((item) => (
                <div key={item.id} style={{ background: "var(--white)", border: `1.5px solid ${item.checked ? "var(--gray-300)" : "var(--gray-200)"}`, borderRadius: "var(--radius-lg)", overflow: "hidden", opacity: item.checked ? 0.65 : 1, transition: "all 0.2s", marginLeft: 8, borderLeft: "4px solid var(--gray-400)" }}>
                  <div style={{ display: "flex", gap: 12, padding: "12px 14px", alignItems: "center" }}>
                    ...
                    <div style={{ width: 54, height: 54, borderRadius: "var(--radius-md)", background: "var(--gray-100)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 22 }}>📝</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontWeight: 700, fontSize: 14, color: "var(--gray-900)", textDecoration: item.checked ? "line-through" : "none", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", margin: 0 }}>{item.name}</p>
