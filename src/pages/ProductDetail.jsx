  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ margin: "16px 16px 0", background: "var(--white)", borderRadius: "var(--radius-lg)", overflow: "hidden", boxShadow: "var(--shadow-sm)" }}>
        {product.image_url ? (
          <img src={product.image_url} alt={product.name} style={{ width: "100%", height: 220, objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} />
        ) : (
        ...
        <div style={{ padding: "16px" }}>
          {product.category && (
            <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, color: "white", background: catColor, borderRadius: "var(--radius-full)", padding: "3px 10px", marginBottom: 8 }}>{product.category}</span>
          )}
            ...
            <div style={{ marginTop: 14, padding: "12px 14px", background: "var(--green-50)", borderRadius: "var(--radius-md)", border: "1px solid var(--green-200)" }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                {lowest.original_price && Number(lowest.original_price) > Number(lowest.price) && (
                  <span style={{ fontSize: 14, color: "var(--gray-400)", textDecoration: "line-through" }}>R$ {Number(lowest.original_price).toFixed(2)}</span>
                )}
                <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 26, color: "var(--green-600)" }}>R$ {Number(lowest.price).toFixed(2)}</span>
                {lowest.discount_pct && (
                  <span style={{ fontSize: 12, fontWeight: 700, color: "white", background: "#ef4444", borderRadius: "var(--radius-full)", padding: "2px 8px" }}>-{lowest.discount_pct}% OFF</span>
                )}
              </div>
              <p style={{ fontSize: 12, color: "var(--green-600)", margin: "6px 0 0" }}>📍 {lowest.supermarket}</p>
              {lowest.valid_until && (
                <p style={{ fontSize: 11, color: "var(--gray-400)", margin: "4px 0 0" }}>Válido até {new Date(lowest.valid_until).toLocaleDateString("pt-BR")}</p>
              )}
            </div>
          )}
        </div>
        ...
        {isLogged && (
          <div style={{ borderTop: "1px solid var(--gray-100)" }}>
            <button onClick={() => inList ? removeFromList(product.id) : addToList(product, lowest?.price, lowest?.supermarket)}
              style={{ width: "100%", padding: "16px", background: inList ? "var(--green-500)" : "var(--white)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer", border: "none" }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: inList ? "white" : "var(--green-600)" }}>{inList ? "Na lista ✓" : "Adicionar à lista"}</span>
            </button>
          </div>
        )}
      </div>
      ...
      <div style={{ margin: "16px 16px 0" }}>
        <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 16, color: "var(--gray-900)", marginBottom: 12 }}>🏪 Preços por mercado ({offers.length})</p>
        {loading ? (
          [1,2,3].map((i) => (
          ...
          return (
            <div key={offer.id} style={{ background: isLowest ? "var(--green-50)" : "var(--white)", border: `1.5px solid ${isUrgent ? "#f97316" : isLowest ? "var(--green-300)" : "var(--gray-200)"}`, borderRadius: "var(--radius-lg)", padding: "14px 16px", marginBottom: 10, display: "flex", alignItems: "center", animation: "fadeUp 0.25s ease both", animationDelay: `${idx*0.05}s` }}>
              <div style={{ flex: 1 }}>
                <button onClick={() => goToMarket({ name: offer.supermarket })}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {isLowest && <span style={{ fontSize: 10, fontWeight: 700, color: "var(--green-700)", background: "var(--green-100)", borderRadius: "var(--radius-full)", padding: "2px 10px", border: "1px solid var(--green-300)", alignSelf: "flex-start" }}>✓ Melhor preço</span>}
                  {!isLowest && diff && <p style={{ fontSize: 11, color: "var(--gray-400)", margin: 0 }}>+R$ {diff} a mais</p>}
                  {offer.valid_until && (
              ...
              <div style={{ textAlign: "right" }}>
                {hasDiscount && <p style={{ fontSize: 12, color: "var(--gray-400)", textDecoration: "line-through", margin: "0 0 2px" }}>R$ {Number(offer.original_price).toFixed(2)}</p>}
                <p style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 22, color: hasDiscount ? "#ef4444" : isLowest ? "var(--green-600)" : "var(--gray-700)", margin: 0 }}>R$ {Number(offer.price).toFixed(2)}</p>
                {hasDiscount && offer.discount_pct && <span style={{ fontSize: 11, fontWeight: 700, color: "white", background: "#ef4444", borderRadius: "var(--radius-full)", padding: "1px 7px" }}>-{offer.discount_pct}% OFF</span>}
              </div>
            </div>
          );
        })}
